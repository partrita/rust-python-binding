use polodb_core::bson::{Bson, Document};
use polodb_core::results;
use pyo3::prelude::*;
use pyo3::types::{PyAny, PyBool, PyBytes, PyDict, PyFloat, PyList, PyString};
use pyo3::IntoPyObjectExt;

pub fn convert_py_list_to_vec_document(py_list_obj: &Bound<'_, PyAny>) -> PyResult<Vec<Document>> {
    if let Ok(py_list) = py_list_obj.cast::<PyList>() {
        let mut docs = Vec::with_capacity(py_list.len());
        for item in py_list.iter() {
            docs.push(convert_py_obj_to_document(&item)?);
        }
        Ok(docs)
    } else {
        Ok(Vec::new())
    }
}

pub fn convert_py_obj_to_document(py_obj: &Bound<'_, PyAny>) -> PyResult<Document> {
    if let Ok(dict) = py_obj.cast::<PyDict>() {
        let mut doc = Document::new();
        for (key, value) in dict.iter() {
            let key: String = key.extract()?;
            let bson_value = convert_py_obj_to_bson(&value)?;
            doc.insert(key, bson_value);
        }
        Ok(doc)
    } else {
        Err(pyo3::exceptions::PyTypeError::new_err(
            "Unsupported Python type for BSON conversion",
        ))
    }
}

pub fn convert_py_obj_to_bson(py_obj: &Bound<'_, PyAny>) -> PyResult<Bson> {
    if let Ok(rust_string) = py_obj.extract::<String>() {
        Ok(Bson::String(rust_string))
    } else if let Ok(rust_bool) = py_obj.extract::<bool>() {
        Ok(Bson::Boolean(rust_bool))
    } else if let Ok(rust_int) = py_obj.extract::<i64>() {
        Ok(Bson::Int64(rust_int))
    } else if let Ok(rust_float) = py_obj.extract::<f64>() {
        Ok(Bson::Double(rust_float))
    } else if let Ok(dict) = py_obj.cast::<PyDict>() {
        let mut bson_doc = Document::new();
        for (key, value) in dict.iter() {
            let key_str: String = key.extract::<String>()?;
            let bson_value = convert_py_obj_to_bson(&value)?;
            bson_doc.insert(key_str, bson_value);
        }
        Ok(Bson::Document(bson_doc))
    } else if let Ok(list) = py_obj.cast::<PyList>() {
        let mut bson_array = Vec::new();
        for item in list.iter() {
            let bson_item = convert_py_obj_to_bson(&item)?;
            bson_array.push(bson_item);
        }
        Ok(Bson::Array(bson_array))
    } else {
        Err(pyo3::exceptions::PyTypeError::new_err(
            "Unsupported Python type for BSON conversion",
        ))
    }
}

pub fn delete_result_to_pydict<'py>(
    py: Python<'py>,
    delete_result: results::DeleteResult,
) -> PyResult<Bound<'py, PyDict>> {
    let py_dict = PyDict::new(py);
    py_dict.set_item("deleted_count", delete_result.deleted_count as i64)?;
    Ok(py_dict)
}

pub fn update_result_to_pydict<'py>(
    py: Python<'py>,
    update_result: results::UpdateResult,
) -> PyResult<Bound<'py, PyDict>> {
    let py_dict = PyDict::new(py);
    py_dict.set_item("matched_count", update_result.matched_count as i64)?;
    py_dict.set_item("modified_count", update_result.modified_count as i64)?;
    Ok(py_dict)
}

pub fn document_to_pydict<'py>(py: Python<'py>, doc: Document) -> PyResult<Bound<'py, PyDict>> {
    let py_dict = PyDict::new(py);
    for (key, value) in doc {
        let py_value = bson_to_py_obj(py, &value)?;
        py_dict.set_item(key, py_value)?;
    }
    Ok(py_dict)
}

pub fn bson_to_py_obj<'py>(py: Python<'py>, bson: &Bson) -> PyResult<Bound<'py, PyAny>> {
    let obj = match bson {
        Bson::Null => py.None().into_bound(py),
        Bson::Int32(i) => i.into_bound_py_any(py)?,
        Bson::Int64(i) => i.into_bound_py_any(py)?,
        Bson::Double(f) => PyFloat::new(py, *f).into_any(),
        Bson::String(s) => PyString::new(py, s).into_any(),
        Bson::Boolean(b) => PyBool::new(py, *b).to_owned().into_any(),
        Bson::Array(arr) => {
            let py_list = PyList::empty(py);
            for item in arr {
                py_list.append(bson_to_py_obj(py, item)?)?;
            }
            py_list.into_any()
        }
        Bson::Document(doc) => {
            let py_dict = PyDict::new(py);
            for (key, value) in doc.iter() {
                py_dict.set_item(key, bson_to_py_obj(py, value)?)?;
            }
            py_dict.into_any()
        }
        Bson::RegularExpression(regex) => {
            let re_module = py.import("re")?;
            re_module
                .call_method1("compile", (regex.pattern.as_str(),))?
        }
        Bson::JavaScriptCode(code) => PyString::new(py, code).into_any(),
        Bson::Timestamp(ts) => (ts.time, ts.increment).into_bound_py_any(py)?,
        Bson::Binary(bin) => PyBytes::new(py, &bin.bytes).into_any(),
        Bson::ObjectId(oid) => PyString::new(py, &oid.to_hex()).into_any(),
        Bson::DateTime(dt) => {
            let timestamp = dt.timestamp_millis() / 1000;
            let datetime = py.import("datetime")?.getattr("datetime")?;
            datetime.call1((timestamp,))?
        }
        Bson::Symbol(s) => PyString::new(py, s).into_any(),
        _ => py.None().into_bound(py),
    };
    Ok(obj)
}
